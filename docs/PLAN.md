...
| 13 | `pipeline/dags/ingest_csv_dag.py` | `task_load` dùng `if_exists='append'` → mỗi lần DAG `@daily` chạy đều append thêm 5 dòng trùng `id` vào `employees` → `data_quality_check` fail với `duplicates=5` | HIGH | Sửa `task_load` theo pattern delete-insert: trước khi `load_to_postgres`, chạy `DELETE FROM employees WHERE id = ANY(:ids)` để xóa dòng trùng id. Dọn dữ liệu cũ: `DELETE WHERE ctid NOT IN (SELECT MAX(ctid) GROUP BY id)` → xóa 5 dòng duplicate |
