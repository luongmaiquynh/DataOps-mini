...
- **17 bug đã phát hiện và fix** trong quá trình kiểm thử (pandas StringIO, PromQL gauge, no-op DAG expression, missing promtail config, alerts.yml not mounted, VM3 node-exporter missing host mounts, ContainerRestartingTooMuch expression, employees duplicates, hello_world_test DAG, CD git conflict, weather_hanoi duplicate data với delete-insert pattern, promtail env var expansion silently failing, v.v.)
