...
| Promtail không đẩy log vào Loki — Grafana Explore Loki hiện "No labels found" | `promtail-config.yml` dùng `${LOKI_URL}` làm biến môi trường nhưng thiếu flag `-config.expand-env=true` trong command → Promtail kết nối đến URL `${LOKI_URL}` theo nghĩa đen thay vì `http://loki:3100/loki/api/v1/push` → không push được log nào | Hardcode URL trực tiếp trong config: đổi `url: ${LOKI_URL}` thành `url: http://loki:3100/loki/api/v1/push` trong `monitoring/promtail/promtail-config.yml` → restart container promtail → Loki nhận log thành công |
| `data_quality_check` fail với `duplicates=5` trong bảng `employees` (lần 2) | `ingest_csv` DAG `@daily` dùng `if_exists='append'` → mỗi lần chạy append thêm 5 dòng trùng `id` vào `employees` | **Bước 1:** `DELETE WHERE ctid NOT IN (SELECT MAX(ctid) GROUP BY id)` → xóa 5 dòng duplicate. **Bước 2:** Sửa `task_load` trong `ingest_csv_dag.py` theo pattern delete-insert — trước khi INSERT, chạy `DELETE FROM employees WHERE id = ANY(:ids)` để xóa dòng trùng id. Sau fix: bảng luôn giữ đúng 5 dòng dù DAG chạy bao nhiêu lần |
...
- **18 bug đã phát hiện và fix** trong quá trình kiểm thử (pandas StringIO, PromQL gauge, no-op DAG expression, missing promtail config, alerts.yml not mounted, VM3 node-exporter missing host mounts, ContainerRestartingTooMuch expression, employees duplicates, hello_world_test DAG, CD git conflict, weather_hanoi duplicate data với delete-insert pattern, promtail env var expansion silently failing, employees duplicate data với delete-insert pattern, v.v.)
